...
bool Background::init() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    spawn_sprites[i].setScale(GameConstants::SPAWN_SIZE.x / spawn_sprites[i].getLocalBounds().width,
  ...
  borderLine = sf::RectangleShape(sf::Vector2f(GameConstants::WINDOW_WIDTH, GameConstants::BORDER_WIDTH));
  borderLine.setFillColor(GameConstants::BORDER_COLOR);
  borderLine.setPosition(0, GameConstants::BORDER_HEIGHT);

  header = sf::RectangleShape(sf::Vector2f(GameConstants::WINDOW_WIDTH, GameConstants::BORDER_HEIGHT));
